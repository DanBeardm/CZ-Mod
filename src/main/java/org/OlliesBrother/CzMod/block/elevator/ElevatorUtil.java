package org.OlliesBrother.CzMod.block.elevator;
...
public final class ElevatorUtil {
    ...
    public static boolean tryTeleport(ServerPlayerEntity player, boolean down) {
        ServerWorld world = player.getServerWorld();

        // Must be standing on elevator (block directly below feet)
        BlockPos originElevator = player.getBlockPos().down();
        if (!world.getBlockState(originElevator).isOf(CzBlocks.ELEVATOR)) return false;
    ...
    private static BlockPos findNextElevator(ServerWorld world, BlockPos origin, boolean down) {
        ...
        for (int i = 1; i <= MAX_RANGE; i++) {
            ...
            if (world.getBlockState(check).isOf(CzBlocks.ELEVATOR)) return check;
        }
        return null;
    }
